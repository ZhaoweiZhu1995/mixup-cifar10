...
def pred_prob_dist(param_dict, model_log_path, model_name = None, low_bound=0., high_bound=1.0, bin_size=0.1, 
    ...
    for i_batch, (inputs, labels, true_labels, raw_idx) in tqdm(enumerate(dataloader)):
        ...
        # print(preds.shape)
        loss_rec = torch.log(probs+1e-8)  + (torch.log(probs+1e-8) - torch.sum(out_prob, dim=1))/9
...
def plot_prob_dist2(name, thresholds, num_count_c, num_count_n):
    fig = plt.figure(figsize=(7,5))
    ax  = fig.add_subplot(1,1,1)

    ax.plot(thresholds[1:], num_count_c, marker='o', label="prob dist")
    ax.plot(thresholds[1:], num_count_n, marker='o', label="prob dist")
...
def get_idx(thre, probs, preds, true_labels, save_dir, sel_idx, is_save = True, fname = None, fmark = None, loss_rec = None, thresholds = None):
    clean_idx = []
    noisy_idx = []
    raw_idx = []
    raw_label = []
    ...
    for i in range(len(probs)):
        if preds[i] == true_labels[i] or i in sel_idx:
            loss_c += [loss_rec[i]]
            # exit()
        else:
            loss_n += [loss_rec[i]]
    ...
    print(f'Number of corrects in clean_idx: {num_corr_clean}, noise rates: {1-num_corr_clean/len(clean_idx)}')
    print(f'Number of corrects in noisy_idx: {num_corr_noisy}, noise rates: {1-num_corr_noisy/len(noisy_idx)}')
    ...
    # save noise labels
    fpath = f'datasets/cifar-10-batches-py/noise_label/prediction_noise_{thre}.pt'
...
if __name__ == "__main__":
    ...
    # model_log_path = "/root/zhuzhw/March/PeerLearning/logs/CIFAR10_before317/resnet_cifar18_pre/CE_CE_4k_2020-03-14-07-35"
    # model_log_path = "/root/zhuzhw/March/PeerLearning/logs/CIFAR10_before317/resnet_cifar18_pre/CE_pred_onlyclean_2020-03-16-12-34/"
    ...
    idx_prob_label = pickle.load( open( f"idx_prob_label_{fmark}.pkl", "rb" ) )
    ...
    sel_idx = dict_save['raw_idx']
    print(f'raw_train label 1 is: {labels[sel_idx[:100]]}')
    ...
    # loss_rec = idx_prob_label[7].astype(float)
    
    num_count_c, num_count_n = get_idx(thre = thre, probs = probs, preds = preds, true_labels = true_labels, save_dir = save_dir, sel_idx = sel_idx, fmark = fmark, loss_rec = loss_rec, thresholds = thresholds)
    ...
    plot_prob_dist(name = 'pred_peer_dist_1', thresholds = thresholds, num_count = num_count)
    plot_prob_dist(name = 'pred_peer_dist_2', thresholds = thresholds, num_count = num_count_2)
    print(num_count_c)
    print(num_count_n)
    plot_prob_dist2(name = 'pred_loss', thresholds = thresholds, num_count_c = num_count_c, num_count_n = num_count_n)
